...
def finger_succ_loc(base,f):
    """
    Location of successor finger f with respect to base.
    """
    return (base + 2**f) % MAX_IDENT

def finger_pred_loc(base,f):
    """
    Location of predecessor finger f with respect to base.
    """
    return (base - 2**f) % MAX_IDENT
...
class Node():
    ...
    def add_known_nodes(self,source_path_len,knodes,queue):
        ...
        for kn in set(updated_knodes).union(old_known):
            ...
            def d_fingers(x,y):
                """
                Find minimal distance of x from a finger of y
                (In the correct orientation)
                """
                dists = []
                for f in SUCC_FINGERS:
                    dists.append(dist_ident(finger_succ_loc(y,f),x))
                for f in PRED_FINGERS:
                    dists.append(dist_ident(x,finger_pred_loc(y,f)))
                return min(dists)

            closest_to_kn = min(new_known,key=lambda nkn:
                    (d_fingers(nkn.ident,kn.ident),nkn.path_len))
